...
class Loja {
  ...
  public static void mostraTabela(){
    Loja loja = new Loja();
    for(int i=0; i<loja.Preco.Count; i++){
      Console.Write("[");
      Loja.mostraMagenta($"{i}");
      Console.WriteLine($"] | {loja.Descricao[i]}");
    }
...
-=-=-=-=-=-=-=-=-=-=";
  ...
  public void mostraAmarelo(string variavel) {
    Console.ForegroundColor = ConsoleColor.Yellow;
    Console.Write(variavel);
    Console.ResetColor();
  }
  public void mostraVermelho(string variavel) {
    Console.ForegroundColor = ConsoleColor.Red;
    Console.Write(variavel);
    Console.ResetColor();
  }

  public static void mostraMagenta(string variavel) {
